...
# ─────────────────────────────────────────────
# HANDLE ?action=start_chat BEFORE routing
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# MAIN APP  (only runs if started == True)
# ─────────────────────────────────────────────
API_KEYS_POOL = []
...
if is_arabic:
    ...
    /* Flip overlay gradient for Arabic */
    .hero-overlay {
...
# ─────────────────────────────────────────────
# QUERY PARAM HOOKS (filter only — start_chat handled above)
# ─────────────────────────────────────────────
url_params = st.query_params
...
<div class="cards-row">
    <div class="target-card">
        ...
        <div class="card-subtext">{"ذهبية، طالب، إقامة" if is_arabic else "Golden, Student, Resident"}</div>
    </div>
    <div class="target-card">
        <div class="card-icon">🚗</div>
        <div class="card-title">{t["svc_driving"]}</div>
        <div class="card-subtext">{"تحويل، تجديد، فحص نظر" if is_arabic else "Convert, Renew, Eye Tests"}</div>
    </div>
    <div class="target-card">
        <div class="card-icon">🏢</div>
        <div class="card-title">{t["svc_business"]}</div>
        <div class="card-subtext">{"مناطق حرة، رخص افتراضية" if is_arabic else "Freezone, Virtual Licenses"}</div>
    </div>
    <div class="target-card">
        <div class="card-icon">🔄</div>
        <div class="card-title">{t["svc_renewals"]}</div>
        <div class="card-subtext">{"هوية إماراتية، تسوية مخالفات" if is_arabic else "Emirates ID, Fine Clearance"}</div>
    </div>
    <div class="target-card">
        <div class="card-icon">❓</div>
        <div class="card-title">{t["svc_faq"]}</div>
        <div class="card-subtext">{"تصفح الدليل الكامل" if is_arabic else "Check the full library"}</div>
    </div>
</div>
""")
...
# ─────────────────────────────────────────────
# LIBRARY DATA — bilingual
# ─────────────────────────────────────────────
all_library_items = [
    {
        "category": "Visa Services",
        "title":          "Student Visa Residency Guide",
        "title_ar":       "دليل إقامة تأشيرة الطالب",
        "badge":          "Residency",
        "badge_ar":       "إقامة",
        ...
        "eligibility_ar": "الطلاب الذين تبلغ أعمارهم 18 عامًا فأكثر، يدرسون في جامعة أو كلية إماراتية معتمدة، برعاية ولي الأمر أو المؤسسة التعليمية.",
        ...
        "checklist_ar":   "<strong>المستندات الأساسية:</strong><br>• خطاب القبول الرسمي<br>• نسخة جواز السفر السارية<br>• شهادة اللياقة الطبية<br>• تفاصيل التأمين الصحي",
        "timeline":       "🕒 10 to 15 working days.",
        "timeline_ar":    "🕒 من 10 إلى 15 يوم عمل.",
        "fees":           "Registration: AED 150. Visa issuance: AED 100/year. Medical test: AED 250.",
        "fees_ar":        "التسجيل: 150 درهم. إصدار التأشيرة: 100 درهم/سنة. الفحص الطبي: 250 درهم."
    },
    {
        "category": "Driving License",
        "title":          "Convert Foreign Driving License to UAE",
        "title_ar":       "تحويل رخصة القيادة الأجنبية إلى إماراتية",
        "badge":          "Conversions",
        "badge_ar":       "تحويل",
        ...
        "eligibility_ar": "حاملو رخصة قيادة سارية من دول معتمدة (دول الخليج، المملكة المتحدة، أمريكا، كندا، الاتحاد الأوروبي، اليابان، سنغافورة، أستراليا) مع تأشيرة إقامة إماراتية سارية.",
        ...
        "checklist_ar":   "<strong>المستندات الأساسية:</strong><br>• رخصة القيادة الأجنبية السارية<br>• ترجمة معتمدة<br>• هوية إماراتية سارية<br>• تقرير فحص النظر",
        "timeline":       "🕒 Same-day service.",
        "timeline_ar":    "🕒 خدمة في نفس اليوم.",
        "fees":           "File opening: AED 200. License issuance: AED 600. Knowledge fee: AED 20.",
        "fees_ar":        "فتح الملف: 200 درهم. إصدار الرخصة: 600 درهم. رسوم المعرفة: 20 درهم."
    },
    {
        "category": "Visa Services",
        "title":          "UAE Golden Visa Eligibility",
        "title_ar":       "أهلية التأشيرة الذهبية الإماراتية",
        "badge":          "Golden Visa",
        "badge_ar":       "تأشيرة ذهبية",
        ...
        "eligibility_ar": "مستثمرو العقارات (2 مليون درهم فأكثر)، رواد الأعمال (500 ألف درهم فأكثر)، المهنيون ذوو المهارات العالية، العلماء، الأطباء، والطلاب المتميزون.",
        ...
        "checklist_ar":   "<strong>المستندات الأساسية:</strong><br>• سند ملكية العقار<br>• شهادة أكاديمية معتمدة<br>• خطابات توصية<br>• تقارير مراجعة الحسابات",
        "timeline":       "🕒 7 to 10 working days.",
        "timeline_ar":    "🕒 من 7 إلى 10 أيام عمل.",
        "fees":           "Nomination: AED 150. 10-year visa: AED 2,800. Emirates ID: AED 1,000.",
        "fees_ar":        "رسوم الترشيح: 150 درهم. تأشيرة 10 سنوات: 2,800 درهم. الهوية الإماراتية: 1,000 درهم."
    },
    {
        "category": "Driving License",
        "title":          "UAE Driving License Renewal",
        "title_ar":       "تجديد رخصة القيادة الإماراتية",
        "badge":          "Renewals",
        "badge_ar":       "تجديد",
        "badge_bg": "#FFFBEB", "badge_color": "#B45309",
        "eligibility":    "All residents and citizens with an active or expired UAE driving license.",
        "eligibility_ar": "جميع المقيمين والمواطنين الحاملين لرخصة قيادة إماراتية سارية أو منتهية الصلاحية.",
        "checklist":      "<strong>Primary documents:</strong><br>• Valid Emirates ID<br>• Current/expired license<br>• Eye test record",
        "checklist_ar":   "<strong>المستندات الأساسية:</strong><br>• هوية إماراتية سارية<br>• الرخصة الحالية أو المنتهية<br>• سجل فحص النظر",
        "timeline":       "🕒 3 to 5 working days; digital version immediate.",
        "timeline_ar":    "🕒 من 3 إلى 5 أيام عمل؛ النسخة الرقمية فورية.",
        "fees":           "Age 21+: AED 300. Under 21: AED 100. Eye test: AED 150–180. Courier: AED 25.",
        "fees_ar":        "فوق 21 سنة: 300 درهم. دون 21: 100 درهم. فحص النظر: 150–180 درهم. التوصيل: 25 درهم."
    },
    {
        "category": "Business License",
        "title":          "Dubai Virtual Company License",
        "title_ar":       "رخصة الشركة الافتراضية في دبي",
        "badge":          "Formation",
        "badge_ar":       "تأسيس",
        ...
        "eligibility_ar": "أصحاب الأعمال العالميون وغير المقيمين من أكثر من 100 دولة معتمدة في قطاعات الإبداع والتقنية والخدمات.",
        "checklist":      "<strong>Primary documents:</strong><br>• Valid passport<br>• Global residency proof<br>• Background screening",
        "checklist_ar":   "<strong>المستندات الأساسية:</strong><br>• جواز سفر ساري<br>• إثبات الإقامة العالمية<br>• فحص السجل الجنائي",
        "timeline":       "🕒 25 to 30 working days.",
        "timeline_ar":    "🕒 من 25 إلى 30 يوم عمل.",
        "fees":           "1-year license: USD 233 (AED 850). Registry fee: USD 100.",
        "fees_ar":        "رخصة سنة واحدة: 233 دولار (850 درهم). رسوم التسجيل: 100 دولار."
    }
]
...
# ─────────────────────────────────────────────
h1 = "عنوان الخدمة"       if is_arabic else "Service Title"
h2 = "معايير الأهلية"     if is_arabic else "Eligibility"
h3 = "المستندات المطلوبة" if is_arabic else "Required Documents"
h4 = "المدة الزمنية"      if is_arabic else "Timeline"
h5 = "الرسوم"             if is_arabic else "Fees"

# ─────────────────────────────────────────────
# TABLE ROWS — bilingual
# ─────────────────────────────────────────────
table_rows = []
for item in filtered_items:
    title     = item["title_ar"]       if is_arabic else item["title"]
    badge     = item["badge_ar"]       if is_arabic else item["badge"]
    eligib    = item["eligibility_ar"] if is_arabic else item["eligibility"]
    checklist = item["checklist_ar"]   if is_arabic else item["checklist"]
    timeline  = item["timeline_ar"]    if is_arabic else item["timeline"]
    fees      = item["fees_ar"]        if is_arabic else item["fees"]

    table_rows.append(
        f"<tr>"
        f"<td style='width:22%;'><strong style='color:#111827; font-size:14.5px; display:block; margin-bottom:8px;'>{title}</strong>"
        f"<span class='table-badge' style='background:{item['badge_bg']}; color:{item['badge_color']};'>{badge}</span></td>"
        f"<td style='width:23%; color:#374151;'>{eligib}</td>"
        f"<td style='width:23%; color:#374151;'>{checklist}</td>"
        f"<td style='width:14%; color:#4B5563; font-weight:500;'>{timeline}</td>"
        f"<td style='width:18%; color:#111827; font-weight:600; text-align:right;'>{fees}</td>"
        f"</tr>"
    )

table_rows_html = "".join(table_rows) if table_rows else f"<tr><td colspan='5' style='text-align:center; padding:40px; color:#9CA3AF;'>{'لا توجد سجلات.' if is_arabic else 'No records found.'}</td></tr>"

st.html(
    "<div class='custom-table-container'>"
    "<table class='custom-table'><thead><tr>"
    f"<th style='width:22%;'>{h1}</th>"
    f"<th style='width:23%;'>{h2}</th>"
    f"<th style='width:23%;'>{h3}</th>"
    f"<th style='width:14%;'>{h4}</th>"
    f"<th style='width:18%; text-align:right;'>{h5}</th>"
    "</tr></thead>"
...
<div class="custom-footer-bar">
    {"© 2026 مساعد الخدمات العامة · نموذج أولي · غير تابع لأي جهة حكومية" if is_arabic else "© 2026 Public Services Assistant · Hackathon Prototype · Not affiliated with any UAE government authority"}
</div>
""")
